...
.zi_zpss_mle <- function(par, values, freq) {
  w <- as.numeric(par[1])
  alpha <- as.numeric(par[2])
  lambda <- as.numeric(par[3])
...
#' @export
residuals.zi_zipfpssFit <- function(object, ...){
...
#' @rdname zi_zipfpssFit
#' @export
fitted.zi_zipfpssFit <- function(object,  ...) {
  dataMatrix <- get(as.character(object[['call']]$data))
  dataMatrix[,1] <- as.numeric(as.character(dataMatrix[,1]))
  dataMatrix[,2] <-as.numeric(as.character(dataMatrix[,2]))
...
#' @rdname zi_zipfpssFit
#' @export
coef.zi_zipfpssFit <- function(object, ...){
...
#' @rdname zi_zipfpssFit
#' @export
print.zi_zipfpssFit <- function(x, ...){
  cat('Call:\n')
  print(x[['call']])
  cat('\n')
  cat('Initial Values:\n')
...
#' @rdname zi_zipfpssFit
#' @export
summary.zi_zipfpssFit <- function(object, ...){
  print(object)
  cat('\n')
  cat('Fitted values:\n')
  print(fitted(object))
...
#' @rdname zi_zipfpssFit
#' @export
logLik.zi_zipfpssFit <- function(object, ...){
...
#' @rdname zi_zipfpssFit
#' @export
AIC.zi_zipfpssFit <- function(object, ...){
...
#' @rdname zi_zipfpssFit
#' @export
BIC.zi_zipfpssFit <- function(object, ...){
  dataMatrix <- get(as.character(object[['call']]$data))
  dataMatrix[,1] <- as.numeric(as.character(dataMatrix[,1]))
  dataMatrix[,2] <-as.numeric(as.character(dataMatrix[,2]))
